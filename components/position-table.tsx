  ...
  return (
    <section className="overflow-x-auto rounded border border-slate-200 bg-white shadow-panel">
      <div className="min-w-[1080px]">
        <div className="grid grid-cols-[70px_100px_1fr_170px_130px_190px] border-b border-slate-200 bg-slate-50 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-steel"><span>Pos</span><span>Menge</span><span>Beschreibung</span><span>Zeichnung</span><span>Preis</span><span>Status</span></div>
        {positions.map((position) =>
          editingId === position.id ? (
            <form key={position.id} action={(formData) => savePosition(position.id, formData)} className="grid grid-cols-[70px_130px_1fr_170px_130px_230px] items-start gap-3 border-b border-slate-100 px-4 py-4 text-sm">
            ...
            <div key={position.id} className="grid grid-cols-[70px_100px_1fr_170px_130px_190px] items-center border-b border-slate-100 px-4 py-4 text-sm">
